...
2. **[`documentation/wela-plus-battery-prd-2026.md`](../../documentation/wela-plus-battery-prd-2026.md)** — the PRD this work is producing. **Design-complete and Codex-reviewed twice** as of 2026-05-29 (the second pass tightened OSS/host boundary, photo lifecycle, item_key contract, RLS SQL conventions, and several doc contradictions; see the *Implementation handoff* section's revisions tables for both passes). No design grilling remains — picking this up means picking up implementation, starting with `/writing-plans` against the *Migrations and rollout* section.
...
2. **Implementation — OSS package build.** New repo `masinyusane/assessment-questions` populated with the 11 Question components, internal hooks (`useToggleMark`), bundled item sets (stubs initially), TypeScript types, design tokens, README + `docs/setup-guide.md` + `docs/llm-prompt-template.md` + `LICENSE` (MIT) + `LICENSE-CONTENT` (CC-BY 4.0). Published to npm as `@masinyusane/assessment-questions@0.1.0`.
...
**Most natural next step — migrations.** Run `/writing-plans` to produce the concrete `.sql` migration files from the PRD's section 2 (additive SQL spec). Plan review. Then `/executing-plans` to apply against `masi-app-sqlite` (project ref `segygjzpujphwvrubusm`), run the EGRA backfill, mirror the schema to the SQLite local store, and update `documentation/rls-sync-contract-map.md` per the project's "RLS is one contract" rule. Verify with `npm run test:release` and a physical-device smoke test.

**Then — OSS package build.** Initialise the new `masinyusane/assessment-questions` GitHub repo using the layout in PRD section 1. Phase 1 of the build: package skeleton, types, design tokens, internal hooks, Patterns A/B/C components, bundled EGRA Letter Sounds item set (Masi already has this content). Phase 2: Patterns D/E/F components, bundled WelaPLUS content stubs (real content waits for pedagogy). Publish `@masinyusane/assessment-questions@0.1.0` to npm with `--access public`.
