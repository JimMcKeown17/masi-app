...
`{ activeProgrammeId, scopes: { programmeAssignment, children, childEaAssignments, childProgrammeEnrollments, childClassMemberships, classes, groups, childrenGroups } }`, each `{ ok, rows, complete, error? }`.
- Embedded scopes inherit their carrier query's `ok` (children carry cea/cpe; the membership query carries classes). Dependent scopes not attempted because a parent failed report `ok: false` with a skipped-dependency reason.
...
Per-scope semantics (the scope predicate mirrors the server query's WHERE clause exactly, using the pull's own `activeProgrammeId` — never the device's assumption):

| Scope (server query) | Acknowledged set | Local rows ended | End column |
| --- | --- | --- | --- |
| children (active `child_ea_assignments` for me ∩ active `child_programme_enrollments` in activeProgramme) | returned child ids | my active synced `child_ea_assignments` whose child has a local active `child_programme_enrollments` row for activeProgramme and whose `child_id` is absent | `unassigned_at` |
| child_programme_enrollments (embedded, activeProgramme only) | returned enrollment ids per returned child | active synced enrollments for (returned child, activeProgramme) absent from that child's returned set | `ended_at` |
| child_class_memberships (active, for returned child ids) | returned membership ids | active synced memberships whose `child_id` is in the returned children set and whose id is absent | `exited_at` |
| groups (active `group_ea_assignments` for me, programme-scoped) | returned group ids | synced non-archived groups with `programme_id = activeProgramme` absent from the set | `archived_at` (local groups read is programme-scoped, so archiving the row is the only lever) |
...
| classes (ClassesContext pull: active `class_ea_assignments` for me + programme) | returned class ids | my active synced `class_ea_assignments` for activeProgramme whose `class_id` is absent | `unassigned_at` — never `classes.archived_at` (class rows are shared via children's memberships; ending my assignment removes it from my class list, which is assignment-scoped) |

Deliberate non-targets:
- `children` rows are never archived by reconcile. A server-archived child arrives WITH `archived_at` set (the children query has no archived filter); an absent child means relationship loss and is handled by the assignment row. A server-hard-deleted child (no-history RPC) also disappears via the ended assignment; the orphaned local row is invisible and harmless.
...
- **Errored scope → no reconcile** (finding #20's plumbing is the prerequisite, which is why they land together).
...
- RLS under-return is indistinguishable from removal client-side; the defense is the per-scope error gate plus the RLS contract tests/probes. Documented in the contract map as a standing hazard of reconcile.
...
- Single-flight: each context's `pullFromServer` dedupes on an in-flight promise ref (concurrent triggers join it). Single-flight + staleness makes a separate debounce timer unnecessary (NetInfo flaps join the in-flight pull; after completion the clock is fresh).
