...
- [ ] RED: extend each sheet's suite (or add one for `LastAttemptedBottomSheet`, which has none) with a pin that its backdrop a11y label and dismiss wiring survive: "Dismiss group picker", "Dismiss letter tracker", "Dismiss last attempted selector". For `LastAttemptedBottomSheet` (R7): **backdrop press and Modal `onRequestClose` call `onCancel`; no visible Cancel button is added**; the footer's Confirm calls `onConfirm(selectedIndex)`. The primitive must receive `onDismiss={onCancel}` so both dismissal paths preserve the load-bearing cancel save.
- [ ] RED (R7): tighten `GroupPickerBottomSheet.test.js`'s loose `/\+\s+Add Group 3/` to the exact string `'+  Add Group 3'`, so the double-space invariant cannot regress green.
- [ ] GREEN: rewrite all three to render through `BottomSheet`, deleting their scaffolding and duplicated styles. `LetterTracker` passes its legend as `headerExtras` and `scrollable={false}`, and **its grid's own `paddingHorizontal: spacing.lg` is REMOVED** now that the primitive supplies it, so its tile math (`screenWidth - spacing.lg * 2`) stays valid with the padding applied exactly once (R4). `LastAttempted` passes its "Selected: ..." + Confirm row as `footer`, its centered grid via `bodyContentStyle`, its hidden scroll indicator via `scrollViewProps`, and `onDismiss={onCancel}`.
- [ ] `LetterTrackerBottomSheet.plan5.test.js` passes **unmodified**; `GroupPickerBottomSheet.test.js` passes with only the R7 regex tightening.
- [ ] Commit: `refactor(ui): sheets render through the shared primitive`
