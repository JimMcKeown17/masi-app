...
1. Read **`CONTEXT.md`** before any UX, schema, authorization, reporting, or naming discussion.
2. Read **`PRD.md`** for current product and architecture requirements.
3. Read **`docs/agent-context/README.md`** and follow its task router to the smallest relevant context
   set. Workstream briefings live in that directory.
4. Read **`documentation/ROADMAP.md`** for what remains and
   **`documentation/build-log.md`** for what shipped and passed.
5. Use **`documentation/README.md`** as the human-facing documentation map.

Load task-specific contracts only when relevant:

- UI work: `documentation/design-system.md` and applicable device gates.
- SQLite/schema work: `src/db/migrations.js`; `documentation/DATABASE_SCHEMA_GUIDE.md` is only a
  teaching primer with a historical worked example.
- RLS, repository payload, outbox, pull, reconcile, or Supabase migration work:
  `documentation/rls-sync-contract-map.md`.
- Hard-to-reverse decisions with real trade-offs: `docs/adr/`, created lazily through
  `grill-with-docs`.

Do not use dated audits, plans, archived files, or PRD checklists as status ledgers.
...
**Ad-hoc SQL against `masi-app-sqlite`** (read-only preflights, verification, disposable-data cleanup — NOT DDL; schema changes go through migrations): use the `sqlite-staging-sql` skill (`.claude/skills/sqlite-staging-sql/`) for the runbook — the `sqlite:staging:query` helper, the 401 auth gotchas, and the non-interactive psql fallback. Always verify the target before a write (the command summary prints `project_ref=` — confirm `segygjzpujphwvrubusm`, not `jcqrlwetutnpuchjoyyd`). The Supabase CLI auto-loads `.env.local` from its working directory and honours `SUPABASE_PROJECT_ID` over `--linked`; avoiding a dotenv wrapper or adding `--workdir` does not isolate it. For every Supabase action, the helper isolates both routes: a fresh temporary cwd contains only a `supabase` symlink to the invoking checkout's `supabase/` directory, and an explicit child-environment allowlist drops inherited `SUPABASE_*` except a nonempty `SUPABASE_ACCESS_TOKEN`, then adds the validated staging password and Expo public mappings. It removes the temporary directory without following the symlink, including on failure. **Raw `supabase` commands must never be run from the repository root while `.env.local` defines `SUPABASE_PROJECT_ID`.** Never source or inject `.env.local` into CLI commands; use the helper for staging operations and run interactive CLI login outside the repository root.
...
- **Standing and active docs** (`PRD.md`, `CONTEXT.md`, `documentation/ROADMAP.md`,
  `documentation/rls-sync-contract-map.md`, DEPLOYMENT.md, active workstream specs, behavior docs
  like the auth-session-resilience notes, and this file) describe the present or active target.
  When a code change contradicts one, update the doc **in the same branch as the code change**. If
  you find one making a claim the code contradicts, fix it immediately or file it.
- **Historical docs** in `documentation/archive/`, plus explicitly point-in-time reviews, audits,
  and implementation plans, preserve what was known then. Never rewrite their body to match later
  reality; add a short dated status note at the top pointing at what superseded them.

The archived 2026-07-12 audit (`documentation/archive/codebase-audit-2026-07-12.md`) found four standing-doc drifts this rule would have prevented; periodic audits verify docs against code with file:line evidence, but the same-branch rule is what keeps the interval clean.

### IMPORTANT: Track Progress and Document Decisions

- **documentation/ROADMAP.md**: Keep only genuinely outstanding work here. Use GitHub issues for
  independently executable implementation checklists. Do not add progress checklists to `PRD.md`.
