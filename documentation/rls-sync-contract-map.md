...
| `child_class_memberships` | `childrenRepository` (incl. `updateChild` class-change reassignment); `childClassMembershipsRepository` | `child_id`, `class_id`, `academic_year_id`, `created_by` | Default upsert; reassignment archives the old active membership (`exited_at`, `archive` op) then inserts a new one (`insert` op) | Active child write access plus class access | Insert after `children` and `classes`; archive before ending child/class EA assignment; on a class change, archive the old membership before inserting the new one (same `updateChild` txn, #35) | `junctionRepositoryGuards.test.js`; `childrenRepository.test.js`; `childClassReassignment.test.js` |
