...
| `child_class_memberships` | `childrenRepository`; `childClassMembershipsRepository` | `child_id`, `class_id`, `academic_year_id`, `created_by` | Default upsert | Active child write access plus class access | Insert after `children` and `classes`; archive before ending child/class EA assignment | `junctionRepositoryGuards.test.js`; `childrenRepository.test.js` |
