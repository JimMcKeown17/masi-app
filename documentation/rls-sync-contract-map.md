...
| Batched upsert | `assessment_items`, `letter_mastery`, `session_attendees`, `time_entries` | `upsert(payloads, { onConflict: 'id' })` in batches of at most 100. A returned or thrown batch failure may spend at most 25 per-record fallback attempts across the entire pass, with concurrency at most five. | Parent row (`assessments` / `children` / `sessions`) must be visible/writable first for tables with parent FKs. Unattempted fallback rows return to pending without failure or retry inflation. |
...
The RLS under-return hazard is closed in code by separating hydration authority from absence authority. The remaining deployment gate is to apply migration `20260714220000` to `masi-app-sqlite` and run `npm run rls:probe`. The probe creates disposable service-role truth, signs in as the matching EA, calls the authenticated RPC, and requires every returned relationship set to match exactly before cleaning up.
