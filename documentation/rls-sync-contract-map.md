...
- Repositories are the only domain write path; the retired storage sidecar no longer creates a second copy that can diverge from SQLite. UI-state merges still use `mergeServerRows` until Sprint 4B replaces them with repository reconciliation and a plain cache refresh.
...
| `children` | `childrenRepository.saveChildRecord` | `id`, `created_by`; active programme and academic year when local save creates relationship rows | Default upsert; hard delete via RPC only. Pull hydration consults owner-scoped pending, failed, and in-flight `hard_delete` outbox ids and suppresses matching child, EA assignment, programme enrollment, and class membership rows before merge or persistence. | `children_select_created_by`; `private.current_user_can_read_child` / write helper | After `classes` only when `class_id` is present; before child relationship rows | `childrenRepository.test.js`; `ChildrenContext.test.js`; `ChildrenContextPendingDelete.integration.test.js`; `mergeServerRows.test.js`; `syncOutboxRepository.test.js`; `sqlitePlan1Migrations.test.js`; live rollback child upsert probe |
