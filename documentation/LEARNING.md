...
Immutable assignment tables sharpen that rule further: batchability can depend on the operation, not only the table. Child, class, and group assignment inserts use deterministic IDs and insert-or-ignore because an older device may retry a row the server already has. Consecutive inserts can therefore share an array request with `ignoreDuplicates: true`. An unassignment or handover is different. It must update lifecycle columns on the existing row, so treating it as insert-or-ignore could report success while silently leaving access active. The batch admission rule permits only `insert` for these tables; lifecycle updates stay per-record with `ignoreDuplicates: false`.
...
### Release boundaries: native capability, runtime identity, and backend readiness move together

A marketing version can look cosmetic, but under Expo's `appVersion` runtime policy it is also an OTA compatibility boundary. Adding the Sentry native SDK while keeping version 1.2.0 would create two binaries with the same runtime label: older devices without the native module and newer devices with it. A later 1.2.0 OTA bundle could then assume native code that some 1.2.0 devices do not contain. Version 1.3.0 separates those capabilities before any preview build exists.

The same release-boundary idea applies to the backend. A client can be automated-green and still be unsafe to distribute if its required RPC is absent. Deploying the reconcile migration, checking remote migration parity, and then exercising the RPC as a real authenticated EA turns a migration file from intent into an operational dependency that has been proved. A successful DDL command alone would not prove the RLS-visible behavior.

Tester provisioning is another capability boundary. Creating an Auth user is not equivalent to creating a usable field account. The app also needs a normalized profile and one active Programme assignment. Conversely, creating seeded classes and rosters requires Head Office identity and deduplication rules that a pilot-account script cannot safely invent. The pilot provisioner therefore owns only zero-class testers and refuses everything else. The mental model is **grant each release tool the smallest complete capability it can prove, then fail closed at the boundary**.

### Business dates and lifecycle verbs are domain contracts

A calendar date is not merely a formatted timestamp. Masi's work is attributed to the South African programme day, so the form that creates a session and the dashboard that counts it must ask the same clock. A device in New York at 21:00 is already in the next South African day. If capture defaults from the device calendar while summaries use `Africa/Johannesburg`, the row saves and syncs perfectly but the completion ring and Home dashboard appear broken. The cure is one domain clock at both write and read boundaries, not a compensating dashboard query.
