...
2. **P0, activation and device proof:** add the sensitive Sentry source-map token to EAS, then run the zero-class and observability device gates (§2/14a, §3). The public DSN/environment/organization/project values are configured in both EAS environments, with distinct preview and production event labels. The class -> children onboarding, durable restart recovery, privacy-hardened crash capture, and explicit sync-failure reporting are built and automated-green on the current branch; real-device behavior and live Sentry delivery remain unverified.
...
- [x] **Nullable session relationship indexes, built 2026-07-14.** SQLite migration v9 and canonical Supabase migration `20260714233000_sync_relationship_indexes.sql` add sparse indexes for `sessions.class_id`, `sessions.group_id`, and `session_attendees.group_id`; real-SQLite query plans select all three. The live backend preflight confirmed the indexes were absent before the migration was authored. Live application remains behind the earlier server-authoritative reconcile migration gate.
