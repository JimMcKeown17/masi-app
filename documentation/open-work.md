...
- **Upsert batching is still incomplete, but the safe child-creation slice closed 2026-07-14** - `BATCHABLE_UPSERT_TABLES` now covers `children` and `child_programme_enrollments` in addition to `assessment_items`, `letter_mastery`, `session_attendees`, and `time_entries`. Multiple child creates/reading-level updates and programme enrollments share bounded array upserts with existing per-record fallback. Class memberships retain reconcile-before-upsert, group memberships still need a collision contract, and immutable assignment inserts require insert-or-ignore, so those tables remain deliberately per-record until their exact semantics are designed and tested. *(ZZ review)*
